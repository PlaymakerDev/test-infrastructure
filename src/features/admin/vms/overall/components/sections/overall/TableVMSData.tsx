...
const rawData: VMSEntry[] = [
  {
    ...
    contractNo: 'สอ.5/2567', guarantee: 'ไม่ค้ำ',
    installPoint: 'กค.1001 - จุดที่ 3 กม.6+300 กันหน้าขึ้น', status: 'ออนไลน์',
    totalSigns: 4, onlineCount: 4, offlineCount: 0,
  },
  {
    id: 'kk1001-2', bureau: 'ส่วนกลาง', roadCode: 'กค.1001',
    projectName: 'โครงการก่อสร้างปรับปรุงถนนสาย กค.1001 จุดที่ 2 แห่ง',
    contractNo: 'สอ.6/2567', guarantee: 'ไม่ค้ำ',
    installPoint: 'กค.1001 - จุดที่ 2 กม.5+800 กันโครงการ', status: 'ออฟไลน์',
    totalSigns: 2, onlineCount: 0, offlineCount: 2,
  },
  {
    id: 'kk1001-3', bureau: 'ส่วนกลาง', roadCode: 'กค.1001(ต่อพฤทธิ์)',
    projectName: 'โครงการปรับปรุงสาย กค.1001 แยกทางหลวง หมายเลข 3344 – ถนนสุนุวิก อ.เมือง จ.สมุทรปราการ',
    contractNo: '2566', guarantee: 'หมดค้ำ',
    installPoint: 'ก.หัลพฤทธิ์: มาจาก ก.นนทบุรีบริหาร', status: 'ออนไลน์',
    totalSigns: 6, onlineCount: 6, offlineCount: 0,
  },
  {
    id: 'pt3010-1', bureau: 'สทช.1 ปทุมธานี', roadCode: 'ปน.3017',
    projectName: 'โครงการพัฒนาประสิทธิภาพการบริหารจัดการระบบประมาณ สัจรรยาตรวจสอบรถจักรยานยนต์ ปน.3010',
    contractNo: 'คค 0709/32/2567', guarantee: 'ไม่ค้ำ',
    installPoint: 'VMS >> ปน.3017', status: 'ออนไลน์',
    totalSigns: 3, onlineCount: 3, offlineCount: 0,
  },
  {
    id: 'su2001-1', bureau: 'สทช.1 ปทุมธานี', roadCode: 'สบ.2001',
    projectName: 'โครงการปรับปรุงสาย สบ.2001 แยกทางหลวงหมายเลข 34 – บ้านลาดกระบัง อำเภอบางพลี จังหวัดสมุทรปราการ',
    contractNo: 'คค 0709/29/2567', guarantee: 'หมดค้ำ',
    installPoint: 'VMS >> สบ.2001 กม.0+020', status: 'ออฟไลน์',
    totalSigns: 2, onlineCount: 0, offlineCount: 2,
  },
]
...
const StatusPill: React.FC<{ status: StatusType }> = ({ status }) => {
  ...
  return (
    <span className='inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs whitespace-nowrap' style={{ border: `1px solid ${color}`, color }}>
      {isOnline ? <TbWifi size={14} /> : <TbWifiOff size={14} />}{status}
    </span>
  )
}
...
const TableVMSData: React.FC<Props> = () => {
  ...
  const columns: ColumnsType<Row> = useMemo(() => [
    ...
    {
      title: 'เลขที่สัญญา', key: 'contractNo', width: 200,
      onCell: (row) => row.kind === 'bureau' ? { colSpan: 0 } : {},
      render: (_: unknown, row: Row) => row.kind !== 'entry' ? null : <span className='inline-flex items-center gap-1.5 whitespace-nowrap'>{row.entry.contractNo}<TbInfoSquareRoundedFilled size={18} className='text-white/50 cursor-pointer hover:text-(--yellow)' /></span>,
    },
