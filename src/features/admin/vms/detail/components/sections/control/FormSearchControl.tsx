...
const FormSearchControl: React.FC<Props> = () => {
  const { control, handleSubmit } = useForm<FormSearchValues>({
    defaultValues: { date: [dayjs(), dayjs()], period: 'ALL', status: 'ALL' },
  })
        ...
        <Col xs={24} sm={24} md={12} lg={12} xl={8} xxl={6}>
          <Controller control={control} name='status' render={({ field }) => (
            <div>
              ...
              <div className='overflow-x-auto'><Segmented block {...field} options={STATUS_OPTIONS} size='large' classNames={{ root: 'min-w-max border! border-(--yellow)!' }} /></div>
            </div>
          )} />
        </Col>
      </Row>
