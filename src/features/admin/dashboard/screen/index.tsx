...
import { RoadIdOverrideContext } from '@/hooks/useRoadId'
import {
  AccidentChart,
  Notification,
  RatioChart,
...
const DashboardContent: React.FC<Props> = () => {
  ...
  // LIVE road scope — starts at the URL's road, then follows the map (search-box
  // pick, breadcrumb ✕, "← ทั่วประเทศ"). Fed to every card through
  // `RoadIdOverrideContext`; the cards whose endpoint honours `road_id` refetch,
  // the rest label themselves as unfiltered. URL is never touched (a
  // `router.replace` remounts BaseMap — same reason as `currentDeptId`).
  const [currentRoadId, setCurrentRoadId] = useState<string | null>(focusRoadId)
  ...
  return (
    <DeptIdOverrideContext.Provider value={currentDeptId}>
      <RoadIdOverrideContext.Provider value={currentRoadId}>
        <div className="relative w-screen h-screen overflow-hidden bg-[#050d1a]">
          ...
          <ReactMap
            ...
            onRoadFilterChange={setCurrentRoadId}
          />
      ...
      </RoadIdOverrideContext.Provider>
