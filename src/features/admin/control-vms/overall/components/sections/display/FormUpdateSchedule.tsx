...
import { APIRequestPostVMSMedia, VMSMediaSchedule } from '@/types/control-vms/vms-api'
...
const FormUpdateSchedule: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    ...
    // Full resync (not prune-only) — matches FormAddDetail's behavior so the
    // DayList always mirrors the current date range in both directions.
    // Pruning-only missed the "widen the range back out" case: a day removed
    // by an earlier narrower range never came back when the range widened
    // again, since there was nothing left in `current` to restore it from.
  ...
  const onSubmit = useCallback((formData: FormValues) => {
    ...
    // Both CREATE and EDIT route through the same confirm-before-write modal —
    // passing `id` tells ContentConfirmCreate to PUT instead of POST on confirm.
    setOpenConfirmCreate({
      open: true,
      ids: formData.vms_ids,
      body,
      id: type === 'EDIT' && id ? id : null,
    })
  }, [setOpenConfirmCreate, id, type, message])
              ...
              <Controller
                ...
                render={({ field }) => (
                  <fieldset>
                    ...
                    <Select
                      {...field}
                      // Disabled on the Select itself, not on Controller — a
                      // Controller-level `disabled` makes RHF drop this field
                      // from the submitted values entirely (like a native
                      // disabled <input>), which crashed `type === 'EDIT'`
                      // submits with `vms_ids: undefined`.
                      disabled={type === 'EDIT'}
                    ...
                    <fieldset>
                      ...
                      <Select
                        ...
                        onChange={(value) => {
                          ...
                          if (value === 'SCHEDULE') {
                            // Default rows with no days picked yet to the full available
                            // range (matches FormAddDetail) — a no-op for EDIT rows whose
                            // `days_of_week` is already seeded from existing data.
                            for (let i = 0; i < fields.length; i++) {
                              const current = getValues(`schedules.${i}.days`) ?? []
                              if (!current.length) setValue(`schedules.${i}.days`, availableDays, { shouldValidate: false })
                            }
                          }
                        }}
