  ...
  <div className='flex flex-col gap-4' style={{ flex: '0 0 30%' }}>
    <div style={{ borderRadius: 20, background: '#191919', padding: 24 }}>
      ...
      <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        <InfoCell icon='/images/Maintenance/icsc1.png' label='ผู้รับจ้าง' value={project.contractor} />
        <InfoCell icon='/images/Maintenance/icsc2.png' label='หน่วยงานรับผิดชอบ' value={project.agency} />
        <InfoCell icon='/images/Maintenance/icsc3.png' label='เลขที่สัญญา' value={project.contractNo} />
        <InfoCell icon='/images/Maintenance/icsc1.png' label='เริ่มต้นการรับประกัน' value={project.warrantyStart} />
        <InfoCell icon='/images/Maintenance/icsc2.png' label='สิ้นสุดการรับประกัน' value={project.warrantyEnd} />
        <InfoCell
          icon='/images/Maintenance/icsc3.png'
    ...
    <div style={{ borderRadius: 20, background: '#191919', padding: 24 }}>
      ...
      <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        <InfoCell icon='/images/Maintenance/icsc2.1.png' label='ประเภทอุปกรณ์' value={device.deviceType} />
        <InfoCell icon='/images/Maintenance/icsc2.2.png' label='จุดติดตั้ง / สายทาง' value={device.installPoint} />
        <InfoCell icon='/images/Maintenance/icsc3.png' label='IP Address' value={device.ipAddress} />
        <InfoCell icon='/images/Maintenance/icsc4-5.png' label='วันที่เริ่มออฟไลน์' value={device.offlineDate || '-'} />
        <InfoCell icon='/images/Maintenance/icsc6.png' label='จำนวนวันออฟไลน์' value={device.offlineDays > 0 ? `${device.offlineDays} วัน` : '-'} />
        {device.hasLive && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
