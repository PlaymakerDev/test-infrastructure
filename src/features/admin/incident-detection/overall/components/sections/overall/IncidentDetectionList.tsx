...
const IncidentDetectionList: React.FC<Props> = () => {
  ...
  const columns: ColumnsType<Row> = useMemo(() => [
    ...
    { title: 'เลขที่สัญญา', key: 'contractNo', width: 180, onCell: (r) => r.kind === 'bureau' ? { colSpan: 0 } : {}, render: (_: unknown, r: Row) => r.kind !== 'entry' ? null : <span className='inline-flex items-center gap-1.5 whitespace-nowrap'>{r.entry.contractNo}<TbInfoSquareRoundedFilled size={18} className='text-white cursor-pointer hover:text-(--yellow)' /></span> },
