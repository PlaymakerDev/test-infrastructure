...
import { TbBuildingBridge, TbShield } from 'react-icons/tb'
    ...
    <Row gutter={[16, 16]}>
      ...
      >
        <div className={`h-full bg-[#FFB1001A] border-2 rounded-2xl p-5 border-(--yellow) ${dim}`}>
          <TbBuildingBridge className='fs-24 text-(--yellow) mb-1' />
          <h3 className='text-(--yellow)'>ไฟประดับสะพานในระบบทั้งหมด</h3>
          <p>
            <span className='fs-24 font-bold'>{stats.total.toLocaleString()}</span> จุดติดตั้ง
          </p>
          <p className='fs-12 text-gray-400'>Active : {fmtNumber(stats.online, 0)} ({fmtNumber(stats.totalPct, 1)}%)</p>
        </div>
      </Col>
      <Col
        xs={24}
      ...
      >
        <div className={`h-full bg-[#05F2DB1A] border-2 rounded-2xl p-5 border-teal-500 ${dim}`}>
          ...
          <p className='fs-12 text-gray-400'>Active : {fmtNumber(stats.inWarrantyActive, 0)} ({fmtNumber(stats.inWarrantyPct, 1)}%)</p>
        </div>
      </Col>
      <Col
        xs={24}
      ...
      >
        <div className={`h-full bg-[#9797971A] border-2 rounded-2xl p-5 border-gray-500 ${dim}`}>
          ...
          <p className='fs-12 text-gray-400'>Active : {fmtNumber(stats.expiredActive, 0)} ({fmtNumber(stats.expiredPct, 1)}%)</p>
        </div>
      </Col>
    </Row>
  )
