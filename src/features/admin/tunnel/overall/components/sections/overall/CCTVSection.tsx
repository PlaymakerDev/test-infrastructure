...
import { useTunnelRandomCameras } from '@/hooks/queries/tunnel'
...
const CCTVSection: React.FC<Props> = (props) => {
  ...
  // `camera` field. Guard both the list level and the per-entry access.
  const cameras = Array.isArray(data?.data) ? data.data.filter((e) => e?.camera) : []
