...
// Zoom level used when flying to a single WIM station matched by the search
...
interface Props {
  /** Current search text from FormSearchWIM — used to find the matching WIM
...
const TrackingMarkerLayer: React.FC<TrackingPosition> = (props) => {
  ...
  // Fly to the first WIM station whose StationName matches the search text —
  // mirrors the same substring match TableWIM's search applies to `name`.
  ...
  useEffect(() => {
    ...
    const match = data?.wim.find((s) => (s.StationName ?? '').toLowerCase().includes(q))
    if (!match) return
  ...
  }, [map, isLoaded, isReady, searchText, data?.wim])
