...
const CamerasSummaryTableCctv: React.FC<Props> = ({ cameras }) => {
  ...
  const columns: ColumnsType<Row> = useMemo(() => [
    ...
    },
    {
      ...
      render: (_: unknown, row: Row) =>
        row.kind === 'camera'
          ? <CountBadge value={row.camera.onlineCount} color='#66AEFF' highlight />
          : null,
    },
    {
      title: 'ออฟไลน์',
      ...
      render: (_: unknown, row: Row) =>
        row.kind === 'camera'
          ? <CountBadge value={row.camera.offlineCount} color='#E94C4C' highlight />
          : null,
    },
  ], [router])
...
export default React.memo<Props>(CamerasSummaryTableCctv)
