...
type SessionJSON = { access_token?: string | null; refresh_token?: string | null; refresh_at?: number; expires_at?: number }
...
async function refreshWithRetry(): Promise<void> {
	...
	for (let i = 0; i < delays.length; i++) {
		...
		try {
			...
			console.warn(
				`[auth] refresh attempt ${i + 1}/${delays.length} failed (${kind}); ${more ? "retrying" : "giving up"
				}`,
			)
		}
	}
