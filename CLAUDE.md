...
Still pure mock/static (hardcoded in `data/*.ts` or inline): `bridge-lighting`, `smart-search`, and any feature not named above. `GET /auth/me` (`AdminService.ts`) is the only non-feature backend call.
...
- `traffic-lighting/` — traffic-light-pole/signal-cabinet electrical monitoring (voltage/amp, lamp equipment). Backend-integrated via `LightingService.ts` — hybrid pattern 2/4 as of 2026-07-19 (see the three-menu audit in Data Fetching).
