...
## Bureau "N โครงการ" badge counts DISTINCT projects (2026-08-25)

Every overall page's bureau divider badge (table view) and grid-view bureau header used to render `items.length` — the number of install-point rows — while labelled "โครงการ". Several install points can share one project/contract (e.g. traffic-volume ปท.3010 จุดที่ 1 + จุดที่ 2 = คค 0709/29/2567), so the badge over-counted — verified live 2026-08-25: 182 bureaus across 10 menus showed a wrong number.

Canonical helpers in `src/features/admin/traffic-volume/shared/utils/groupByBureau.ts` (already the cross-feature home — crosswalk/tunnel imported it before):
- `projectKey(...candidates)` — first non-empty of (projectId, contractNo, …); trims blanks; numeric 0 is a real id.
- `countDistinctProjects(items, keyOf)` — distinct keys + identity-less rows counted individually (never collapses unrelated rows).
- `groupByBureau(projects, projectKeyOf)` — the accessor is a REQUIRED second arg on purpose; an accessor-less call is how the rows-not-projects bug happened. Regression tests: `groupByBureau.test.ts`.

All 16 render sites use them (traffic-volume×2, crosswalk, tunnel table + its export flatten, cctv table+grid, incident×2, traffic-signal×2, traffic-lighting, lpr, vms, bridge-lighting, shared `ProjectCardGrid`). Grid bureau pills are WHITE (`1px solid #fff`) matching the table dividers — `ProjectCardGrid`'s yellow pill was the odd one out and was normalized 2026-08-25 on user request. Any new bureau-grouped surface must count via these helpers, not `.length`, and use the white pill.
...
**⚠ SearchBar renders the export button BY DEFAULT** (`src/components/searchable/SearchBar.tsx`: `showExportButton = true`, hidden only in `mode='default'`). Any new page using SearchBar (or the shared FilterBar* toolbars) MUST either pass `onExport` (wire it through ExportFileModal per the conventions below) or explicitly pass `showExportButton={false}` — otherwise it ships a dead button (this exact miss caused two full wiring sweeps). Pages that deliberately hide it today: traffic-lighting overall (wiring exists — flip the flag to enable). statistics detail status was enabled 2026-08-24; incident/alert were already live.

**Detail-header export points** (no SearchBar on the page — the button rides `DetailTitleSection`'s `onExport` prop instead): **bridge-lighting detail** (added 2026-08-24) mounts `ExportBridgeLightingModal` from `screen/index.tsx` (the data owner). Scope was deliberately narrowed by the user the same day to **the two chart datasets only** — one table/sheet per on-screen chart (`แรงดันไฟฟ้าภายในตู้ควบคุม 24 ชั่วโมง (Volt)` / `กระแสไฟฟ้าภายในตู้ควบคุม 24 ชั่วโมง (Amp)`), columns `เวลา + Phase 1–3` mirroring `VOLTAGE_LINES`/`CURRENT_LINES` exactly (no Avg column — that figure belongs to the VoltageStat cards, not the charts). An earlier revision also exported install-point info / per-device shelly status / latest meter reading / 24-h min-avg-max; **that was dropped on request — don't re-add it without asking.** Excel writes raw numbers (sortable/chartable) while the PDF formats via `fmtNumber`; Excel tab names use short forms because the util truncates at 31 chars, with the full chart title in each sheet's header block. Migrate the other bespoke detail headers to this prop rather than hand-rolling a header button.
...
- **`src/utils/export/excel.ts`** — `exportExcel({filenameBase, sheetName, columns, rows, title?, filterNote?})`. Since 2026-07-22 every sheet carries a PDF-style header block above the table (merged+centered bold title, "ข้อมูล ณ วันที่ … เวลา … น." line, optional filterNote line, then a styled header row + bordered cells) — pass `title` with the SAME string the page gives `exportTablePdf`, and the same `filterNote`. All 38 call sites do this; don't add a new one without it. Implementation uses **`xlsx-js-style`** (drop-in SheetJS fork; the community `xlsx` build silently drops font/fill/alignment styles — do not import plain `xlsx` here). For a report that needs SEVERAL differently-shaped tables in one workbook, use **`exportExcelSheets({filenameBase, sheets})`** with one **`excelSheet({sheetName, columns, rows, title?, filterNote?})`** per tab (`excelSheet` erases the per-sheet `Row` generic so unrelated row types can share a workbook without `any`); `exportExcel` is now a thin single-sheet wrapper over it, so every existing call site keeps its exact layout — regression-locked by `src/utils/export/excel.test.ts`.
